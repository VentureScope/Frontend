...
const FutureRolesForecastChartView = dynamic(
  () => import("@/components/new-roadmap/FutureRolesForecastChartView"),
  {
    ssr: false,
    loading: () => (
...
export function FutureRolesForecastChart({
  bars,
  selectedId,
  onSelect,
  meta,
  ...
  return (
    <div
      className={cn(
        "rounded-[24px] border border-border bg-card p-3 sm:p-6",
        className,
      )}
    >
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="max-w-2xl text-sm text-muted-foreground">
            All forecasted roles ranked by average predicted monthly postings.
            Click a bar to choose your roadmap target.
          </p>
        </div>
        {selected ? (
      ...
      {selected && selected.monthlyPostings.length > 0 ? (
