...
import { OrganizationProfilePageSkeleton } from "@/components/organization/OrganizationSkeletons";
...
import { useMyOrganizationMemberContexts } from "@/hooks/useMyOrganizationMemberContexts";
...
export function MyOrganizationMemberProfileView() {
  ...
  const { contexts, organizations, loading, error } =
    useMyOrganizationMemberContexts(orgFilter);
      ...
      {contexts.length === 0 ? (
        <div className="vs-surface mt-10 rounded-md border border-dashed border-border px-6 py-14 text-center">
          <p className="text-sm font-medium text-foreground">
            {selectedOrgName
              ? `You are not a member of ${selectedOrgName}`
              : "Join an organization to see your contextual profile"}
          </p>
          <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
            Once you are a member, we surface skills, roadmap links, and developer
            signals relative to teammates in the same role.
          </p>
          <Link
        ...
        <div className="mt-10 space-y-10">
          {contexts.map(({ org, ctx, techStacks }) => (
            <OrgMemberProfileSection
              key={ctx.orgId}
              ctx={ctx}
              org={org}
