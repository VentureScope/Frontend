...
export default function ResumePreview({ resume = mockResumes[0] }: { resume?: Resume }) {
  ...
  return (
    <div className="resume-print-surface w-full max-w-full overflow-hidden rounded-3xl border border-border bg-card shadow-[0_32px_64px_-12px_rgba(0,0,0,0.14)] lg:rounded-[40px]">
      ...
      <div className="space-y-8 p-5 pb-6 sm:space-y-10 sm:p-8 sm:pb-7 lg:space-y-12 lg:p-12 lg:pb-8 xl:p-14">
        ...
        <div className="space-y-4 text-center sm:space-y-6">
          <h2 className="wrap-break-word text-2xl font-black uppercase leading-tight tracking-tight text-foreground print:text-slate-900 sm:text-3xl lg:text-[38px] lg:leading-none">
            {resume.title}
          </h2>
            ...
            {resume.content.experience.map((job) => (
              <div key={job.id} className="space-y-4">
                <div className="flex flex-col gap-1.5 sm:flex-row sm:items-baseline sm:justify-between">
                  <h5 className="wrap-break-word text-[15px] font-bold text-foreground print:text-slate-900 sm:text-[15.5px]">
      ...
      <div className="no-print flex flex-col gap-3 border-t border-border/50 p-5 pt-5 sm:flex-row sm:gap-4 sm:p-8 sm:pt-6 lg:p-12 xl:p-14">
        <button
          type="button"
          className="flex h-12 flex-1 items-center justify-center gap-3 rounded-full bg-primary/10 text-xs font-bold text-primary shadow-sm transition-colors hover:bg-primary/15 sm:h-14 sm:text-sm"
        >
        ...
        <button
          ...
          className="flex h-12 flex-[1.4] items-center justify-center gap-3 rounded-full bg-foreground text-xs font-bold text-background shadow-xl shadow-foreground/10 transition-all hover:bg-foreground/90 sm:h-14 sm:text-sm"
        >
