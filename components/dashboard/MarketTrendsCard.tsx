...
export default function MarketTrendsCard() {
  ...
  return (
    <div className="rounded-3xl sm:rounded-[40px] border border-border bg-card p-6 sm:p-8 lg:p-10 shadow-sm">
      <div className="mb-6 sm:mb-10 flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <span className="rounded-full bg-primary/10 px-3 py-1 text-[9px] sm:text-[10px] font-bold text-primary uppercase">
            React
          </span>
          <span className="rounded-full bg-primary/10 px-3 py-1 text-[9px] sm:text-[10px] font-bold text-primary uppercase">
            Node.js
      ...
      <div className="flex items-end justify-between gap-2 sm:gap-4 h-32 sm:h-48">
        {bars.map((h, i) => (
          <div
            key={i}
            className="w-full rounded-lg sm:rounded-xl bg-primary transition-all hover:bg-primary/90"
